...
export function TestRideForm({
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    if (
      ...
      e.preventDefault();
      setError(
        'Please fill in your name, mobile number, and chosen scooter model.'
      );
      return;
    }
    ...
    // The success page is handled by FormSubmit after submission.
    if (onSuccess) {
      onSuccess();
    }
  };
      ...
      {submitted ? (
        <div className="text-center py-8 px-4 animate-in fade-in duration-300">
          ...
          <p className="text-xs sm:text-sm text-[#E8B7B7] max-w-md mx-auto leading-relaxed mb-6">
            Thank you,{' '}
            <strong className="text-white">{formData.name}</strong>. Your test
            ride interest for{' '}
            <strong className="text-[#F9040C]">
              {formData.scooterModel}
            </strong>{' '}
            has been received by our showroom team.
          </p>

          <div className="p-3.5 rounded-xl bg-[#050505] border border-[#2C0F12] text-xs text-[#E8B7B7] max-w-md mx-auto mb-6 text-left">
            <span className="text-[#F9040C] font-bold block mb-1">
              Showroom Verification Note:
            </span>
            Please remember to carry a valid government ID or driving license
            when visiting the showroom for your test ride.
          </div>
        ...
        >
          ...
          <input
            type="hidden"
            name="_subject"
            value="New Test Ride Request - Patel Automobiles"
          />
          ...
          <div className="text-left mb-6">
            ...
            <p className="text-xs text-[#E8B7B7] mt-1">
              Experience zero-noise electric acceleration firsthand at Patel
              Automobiles Lailunga.
            </p>
          </div>
          ...
          <div>
            <label
              ...
              className="block text-xs font-bold text-[#FCE9E9] uppercase tracking-wider mb-1.5"
            >
              Full Name *
            </label>
          ...
          <div>
            <label
              htmlFor="test-ride-scooter-model"
              className="block text-xs font-bold text-[#FCE9E9] uppercase tracking-wider mb-1.5"
            >
          ...
          <p className="text-[11px] text-center text-[#E8B7B7] pt-1">
            * Test rides are subject to slot availability and vehicle battery
            charging readiness at the dealership.
          </p>
        </form>
      )}
    </div>
